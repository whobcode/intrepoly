- > i was able to use mwc55 without putting in a password, only whobcode13 shouldbe able to do that, the sign up should be an account creation and the email flow should follow that, create a
 standalone script that incorporates a production ready sign up flow, with email verification and auto replys, as well as a "user logged in succesfully" with username in the body of a 
production ready signin@hwmnbn.me flow ,  secondly if its only 1 player why hasnt the game filled the remaining 7 places with an ai agents, can you create them with "npm i -g agents" and 
then make agents from those free models in the dropdown? the "go arrow" svg needs to point in the oppposite direction, the chatbox isnt visible did you ever incorporate that like i asked? 
i would also like the agents to be able to utilize that chat box, where is the realtime kit video implementation like we have in the project olla.hwmnbn.me, small video chat boxes should 
fit under the logs section for when 8 or more people join, videos come up per camera tho so if only one active then only one shows.
- /**
